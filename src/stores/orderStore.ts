...
export interface IOrderStore {
    ...
    // 确认打印
    comfirmOrder: (orderId: string) => Promise<any>
}
    ...
    async fetchOrderData(params) {
        ...
        if(params.type === 1) {
            ...
            runInAction(() => {
                ...
                this.orderListData.page = reqParams.pages
                this.orderListData.num = reqParams.number
            ...
            runInAction(() => {
                // 填入产品map对象
                this.orderData.merge(resData.list.map(item => [item.order_number, item]))
